...
export function ProjectBulkImport(props: ProjectBulkImportProps) {
  ...
  function parseCSV(text: string): Omit<ParsedImport, "alreadyExists" | "selected">[] {
    ...
    for (let i = startIndex; i < lines.length; i++) {
      ...
      const rawType = (matches[2]?.replace(/^"|"$/g, "").trim().toLowerCase() as any) || "string";
      const rawScope = (matches[3]?.replace(/^"|"$/g, "").trim().toLowerCase() as any) || "all";
      
      const cleanType: any = ["string", "number", "boolean", "json"].includes(rawType) ? rawType : "string";
      const cleanScope: any = ["client", "server", "all"].includes(rawScope) ? rawScope : "all";
